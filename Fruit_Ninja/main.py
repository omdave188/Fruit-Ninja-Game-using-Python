...
fruits = ['melon', 'orange', 'pomegranate', 'guava','apple','strawberry' , 'bomb']    #entities in the game
sounds = ['1' , '2' , '3' , '4' ]
...
background = pygame.image.load('background.jpg') #game background
...
def generate_random_fruits(fruit):
    ...
    if random.random() >= 0.75:     #Return the next random floating point number in the range [0.0, 1.0) to keep the fruits inside the gameDisplay
        ...
        throw_sound = mixer.Sound('sounds/fruit spawn.wav')
...
def hide_cross_lives(x, y):
    gameDisplay.blit(pygame.image.load("images/failed_lives.png"), (x, y))
...
def show_gameover_screen():
    ...
    game_start = mixer.Sound('sounds/game start.wav')
...
while game_running :
    if game_over :
        ...
        game_over = False
        player_lives = 3
        draw_lives(gameDisplay, 690, 5, player_lives, 'images/failed_lives.png')
        score = 0
    ...
    for key, value in data.items():
        if value['throw']:
            ...
            if not value['hit'] and current_position[0] > value['x'] and current_position[0] < value['x']+60 \
                ...
                if key == 'bomb':
                    player_lives -= 1
                    bomb_explode = mixer.Sound('sounds/explode.wav')
                    bomb_explode.set_volume(0.10)
                    bomb_explode.play()
                    if player_lives == 0:
                    ...
                    if player_lives == 0 :
                        show_gameover_screen()
                        game_over = True
                    gameDisplay.blit(slash, (value['x'], value['y']))
                    half_fruit_path = "images/explosion.png"
                else:
                    gameDisplay.blit(slash, (value['x'],value['y']))
                    half_fruit_path = "images/" + "half_" + key + ".png"
                    fruit_cut_path = "sounds/" + "splatter_" + sound + ".wav" 
                    fruit_cut = mixer.Sound(fruit_cut_path)
                    fruit_cut.set_volume(0.1)
                    fruit_cut.play()
